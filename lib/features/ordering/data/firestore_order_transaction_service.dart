...
import '../../../domain/models/model_serializers.dart';
...
class FirestoreOrderTransactionService implements OrderTransactionService {
  ...
  @override
  Future<void> clearPaidTable(RestaurantTable table) async {
    ...
    try {
      await _firestore.runTransaction((transaction) async {
        final tableSnapshot = await transaction.get(tableRef);
        final remoteTable = tableSnapshot.data();
        if (!tableSnapshot.exists || remoteTable == null) {
          throw const OrderTransactionFailure(
            'Không tìm thấy bàn cần dọn trên Firestore.',
          );
        }

        final remoteStatus = remoteTable['trangThai']?.toString();
        final remoteOrderId = remoteTable['orderHienTaiId']?.toString();
        final hasRemoteOrder =
            remoteOrderId != null && remoteOrderId.isNotEmpty;

        // Dọn bàn là idempotent để hai thiết bị cùng nhận một kết quả ổn định.
        if (remoteStatus == TableStatus.available.name && !hasRemoteOrder) {
          return;
        }
        if (remoteStatus != TableStatus.paid.name) {
          throw const OrderTransactionFailure(
            'Bàn chưa được thanh toán hoặc đã thay đổi trên thiết bị khác.',
          );
        }
        if (table.currentOrderId != null &&
            hasRemoteOrder &&
            remoteOrderId != table.currentOrderId) {
          throw const OrderTransactionFailure(
            'Order của bàn đã thay đổi; vui lòng tải lại dữ liệu.',
          );
        }
      ...
      throw OrderTransactionFailure(
        error.message ?? 'Không thể dọn bàn an toàn trên Firestore.',
      );
    }
  }
}
